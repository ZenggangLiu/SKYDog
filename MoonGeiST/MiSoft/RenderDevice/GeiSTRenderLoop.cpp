...
/// Library headers
#include "CD3D12Helper/CD3D12Helper.hpp"
#include "CD3D12Helper/CD3D12BlendDesc.hpp"
#include "CD3D12Helper/CD3D12DescriptorHandle_CPU.hpp"
#include "CD3D12Helper/CD3D12HeapProperty.hpp"
#include "CD3D12Helper/CD3D12RasterizerDesc.hpp"
#include "CD3D12Helper/CD3D12ResourceBarrier.hpp"
#include "CD3D12Helper/CD3D12ResourceDesc.hpp"
#include "CD3D12Helper/CD3D12RootSignature.hpp"
#include "CD3D12Helper/CD3D12ShaderByteCode.hpp"
#include "CD3D12Helper/CD3D12TextureCopyLocation.hpp"
...
GeiSTRenderLoop::GeiSTRenderLoop(
    ...
    m_viewport(0.0f, 0.0f, (float)win_width, (float)win_height),
    m_scissorRect{ 0, 0, (LONG)win_width, (LONG)win_height },
...
UINT GeiSTRenderLoop::GetWidth() const
{
    return m_win_width;
}

UINT GeiSTRenderLoop::GetHeight() const
{
    return m_win_height;
}

const WCHAR* GeiSTRenderLoop::GetTitle() const
{
    return m_win_title.c_str();
}

void GeiSTRenderLoop::ParseCommandLineArgs(
    WCHAR* argv[], int argc)
{
    for (int i = 1; i < argc; ++i)
    {
            ...
            _wcsnicmp(argv[i], L"/warp", wcslen(argv[i])) == 0)
        {
            ...
            m_win_title = m_win_title + L" (WARP)";
        }
    }
}

void GeiSTRenderLoop::OnInit()
{
    LoadPipeline();
    LoadAssets();
}
...
std::wstring GeiSTRenderLoop::GetAssetFullPath(LPCWSTR assetName)
{
...
void GeiSTRenderLoop::GetHardwareAdapter(
...
void GeiSTRenderLoop::SetCustomWindowText(LPCWSTR text)
{
    std::wstring windowText = m_win_title + L": " + text;
    SetWindowText(GeiSTApplication::GetHwnd(), windowText.c_str());
}

// Load the rendering pipeline dependencies.
void GeiSTRenderLoop::LoadPipeline()
{
    ...
    swapChainDesc.Width = m_win_width;
    swapChainDesc.Height = m_win_height;
    ...
    ThrowIfFailed(factory->MakeWindowAssociation(GeiSTApplication::GetHwnd(), DXGI_MWA_NO_ALT_ENTER));
...
void GeiSTRenderLoop::LoadAssets()
{
    ...
    // Create the pipeline state, which includes compiling and loading shaders.
    {
        ...
        D3D12_INPUT_ELEMENT_DESC inputElementDescs[] =
        {
            ...
            { "TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT, 0,
              sizeof(Vertex::position), D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0
            },
        };
    ...
    // Create the vertex buffer.
    {
        ...
        {
            {
                { 0.0f, 0.25f * m_aspectRatio, 0.0f }, /// Pos
                { 0.5f, 0.0f }                         /// UV
            },
        ...
        const CD3DX12_HEAP_PROPERTIES upload_heap_type(D3D12_HEAP_TYPE_UPLOAD);
        const CD3DX12_RESOURCE_DESC vertex_buffer_desc = CD3DX12_RESOURCE_DESC::Buffer(vertexBufferSize);
    ...
    // Create the texture.
    {
        ...
        const CD3DX12_HEAP_PROPERTIES default_heap_type(D3D12_HEAP_TYPE_DEFAULT);
        ThrowIfFailed(m_device->CreateCommittedResource(
            &default_heap_type,
            D3D12_HEAP_FLAG_NONE,
            &textureDesc,
        ...
        const CD3DX12_HEAP_PROPERTIES upload_heap_type(D3D12_HEAP_TYPE_UPLOAD);
        const CD3DX12_RESOURCE_DESC upload_buffer_desc = CD3DX12_RESOURCE_DESC::Buffer(uploadBufferSize);
        ThrowIfFailed(m_device->CreateCommittedResource(
            &upload_heap_type,
            D3D12_HEAP_FLAG_NONE,
            &upload_buffer_desc,
        ...
        const CD3DX12_RESOURCE_BARRIER resource_barrier =
...
std::vector<UINT8> GeiSTRenderLoop::GenerateTextureData()
{
...
void GeiSTRenderLoop::OnUpdate()
{
}

// Render the scene.
void GeiSTRenderLoop::OnRender()
{
...
void GeiSTRenderLoop::OnDestroy()
{
...
void GeiSTRenderLoop::OnKeyDown(
    const UINT8 key)
{

}

void GeiSTRenderLoop::OnKeyUp(
    const UINT8 key)
{

}

void GeiSTRenderLoop::PopulateCommandList()
{
...
void GeiSTRenderLoop::WaitForPreviousFrame()
{
